...
Native macOS 15+ SwiftUI app (Apple Silicon / MLX) + 14 local Swift packages. The repository contains exactly 14 local Swift packages, and protected CI tests that fixed inventory. Read
...
- Building/testing requires macOS + Xcode (`xcodebuild -workspace SupraAI.xcworkspace
  ...
  `bash Scripts/test-all-packages.sh` before merge;

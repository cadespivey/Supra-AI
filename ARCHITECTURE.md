...
- Exhaustive corpus work uses a separate v064 coverage ledger rather than ranked retrieval.
  ...
  The Review workbench exports one deterministic managed CSV snapshot of every saved finding, generated and local-user-authored value, review and support state, project staleness, and ordered frozen supporting and contrary sources from a single Store read transaction. Only after the validated file installs atomically does Store record a nil-Structured-Output Review export plus a project audit, so contrary-only Review work remains exportable without weakening Structured Output export eligibility.

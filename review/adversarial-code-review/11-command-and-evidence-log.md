...
| RuntimeClient TSan and ASan | 4/4 passed under each sanitizer at the initial integrated snapshot `19e06b451cd585f7b7b360ea916e992339b46845`; this was not rerun at `f24c792`. |
...
### Final 2.2.1, signed-smoke, and CI-portability rerun

| Command / evidence | Result summary |
|---|---|
| Final source identity | `f24c792179137557ae61f259f18a209c9790b345`; source changes were clean before final evidence-document edits. |
...
| Release-hardening content-bound hosted lifecycle selector | 1/1 passed at `fce83eb` with the exact model fingerprint attested through XPC. xcresult: `/tmp/SupraAI-XPC-GREEN-FINAL/Logs/Test/Test-SupraAI-2026.07.13_13-52-41--0400.xcresult`. The earlier full hosted selector pair passed 2/2 before the snapshot-cleanup-only hardening commits. |
| Release-hardening vertical-window regression selectors | 2/2 passed at `fce83eb`: `testBlockedDraftIsAnnouncedWithoutFileActions` and `testLegacyOutputWarningAnnouncesStatusAndUnavailableExport`. Both assert stable window y-origin and height. xcresult: `/tmp/SupraAI-XPC-GREEN-FINAL/Logs/Test/Test-SupraAI-2026.07.13_13-56-21--0400.xcresult`; one Xcode-internal QoS runtime warning remains recorded. |
| Main-actor portability RED/GREEN | RED `0a452dd`: the scoped static gate found three live-resize observers and zero synchronous main-actor bridges. GREEN `3a55987`: all three callbacks use `MainActor.assumeIsolated`, preserving ordering and lifetime while compiling under Xcode 16.4. |
| Deterministic output-navigation RED/GREEN | RED `ddda8b6`: 0/1 because the application ignored the deterministic output command; xcresult `/tmp/SupraAI-OUTPUT-NAV-RED-3a55987.xcresult`. GREEN `f24c792`: 1/1 through the production `NavigationStack` and detail destination; xcresult `/tmp/SupraAI-OUTPUT-NAV-GREEN-ddda8b6.xcresult`. The test still requires the output row to exist and be hittable; it does not claim direct synthesized-click coverage. |
| Final local app/UI/XPC smoke | 5/5 passed at `f24c792`, including both vertical-window stability selectors and both hosted XPC selectors. xcresult: `/tmp/SupraAI-FULL-SMOKE-f24c792.xcresult`; one Xcode-internal QoS priority-inversion warning remains recorded. |
| [Protected macOS CI run 29275806317](https://github.com/cadespivey/Supra-AI/actions/runs/29275806317) | Exact head `f24c792179137557ae61f259f18a209c9790b345`; 22/22 jobs passed. The Xcode 16.4 Debug and Release app/XPC builds, all 14 package jobs, and the 5/5 app/UI/hosted-XPC smoke passed. Model-catalog run `29275806331` also passed. |
...
| Current Debug and Release build evidence | The exact final source built unsigned Debug and Release app/XPC configurations under GitHub's Xcode 16.4 jobs. A local Xcode 27 Debug app/XPC build also succeeded as part of the final smoke. No changed-source compiler failure occurred. |

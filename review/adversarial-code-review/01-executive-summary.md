...
Final verification ran against source snapshot `f24c792179137557ae61f259f18a209c9790b345`, the 2.2.1 build 387 candidate. The final local app/UI/XPC smoke passed 5/5, including both vertical-window stability selectors, and recorded one Xcode-internal QoS priority-inversion warning. Exact-head protected macOS CI passed 22/22 jobs under GitHub's Xcode 16.4 build environment, and the model-catalog workflow passed. The content-bound hosted XPC lifecycle passed, all 14 package suites passed, Debug and Release app/XPC builds passed, and the deterministic claims, release-transaction, migration, website, and XPC-boundary gates passed. Release qualification now requires repository-owned signed-smoke code, exact model/app/source evidence, and a dedicated ephemeral isolated runner. RuntimeClient and the hosted lifecycle sanitizer results remain recorded at the earlier integrated snapshot; they were not relabeled as rerun at this later SHA. These source-level results do not change the external signed-release qualifications above.

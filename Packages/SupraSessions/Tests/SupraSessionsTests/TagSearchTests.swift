...
final class TagSearchTests: XCTestCase {
    ...
    func testGlobalTagSearchSpansChatsAndNotesAcrossMatters() async throws {
        ...
        try store.scratchPad.addEntry(dayID: day.id, text: "Call client #urgent for @McKernon", mentions: [matter.id], tags: ["urgent"])
        ...
        XCTAssertTrue(hits.contains { $0.kind == .chat && $0.openableChatID == nil && $0.group == "McKernon Motors v. Liberty Rail" })
        ...
        XCTAssertTrue(hits.contains { $0.kind == .note && $0.title.contains("2026-06-14") && $0.group == "McKernon Motors v. Liberty Rail" })
    }

    func testMatterScopedTagSearchIsBoundedToThatMatter() async throws {
        let store = try SupraStore.inMemory()
        let acme = try store.matters.createMatter(name: "McKernon Motors v. Liberty Rail")
        let other = try store.matters.createMatter(name: "Hessington Oil v. Gillis Industries")
        let acmeChat = try store.chats.createMatterChat(matterID: acme.id, title: "McKernon chat")
        _ = try store.chats.appendUserMessage(chatID: acmeChat.id, content: "#urgent mckernon")
        ...
        // Only McKernon's chat and McKernon's note — not the other matter's, not the global chat.
        ...
        XCTAssertFalse(hits.contains { $0.title.contains("Hessington") })
    }

...
final class BenchmarkBaselineContractTests: XCTestCase {
    ...
    func testFrozenBaselineMatchesCurrentHarnessFixturesAndThresholdLedger() throws {
        ...
        let contextUtilization = try measurement("B-CTX-01", "context_utilization", in: baseline)
        XCTAssertEqual(contextUtilization.numerator, 620)
        XCTAssertEqual(contextUtilization.denominator, 700)
        let estimateError = try measurement("B-CTX-01", "fallback_estimate_error", in: baseline)
        XCTAssertEqual(estimateError.numerator, 20)
        XCTAssertEqual(estimateError.denominator, 620)
        let omissionRate = try measurement("B-CTX-02", "responsive_candidate_omission_rate", in: baseline)
        XCTAssertEqual(omissionRate.numerator, 1)
        XCTAssertEqual(omissionRate.denominator, 3)
        let overflowRecovery = try measurement("B-CTX-02", "overflow_recovery_rate", in: baseline)
        XCTAssertEqual(overflowRecovery.value, 1)
        XCTAssertEqual(overflowRecovery.numerator, 1)
        XCTAssertEqual(overflowRecovery.denominator, 1)
        let silentOverflows = try measurement("B-CTX-02", "silent_overflow_count", in: baseline)
        XCTAssertEqual(silentOverflows.value, 0)
        XCTAssertEqual(silentOverflows.numerator, 0)
        XCTAssertEqual(silentOverflows.denominator, 1)
    ...
    private func harnessDigest(root: URL) throws -> String {
        let paths = [
            ...
            "Packages/SupraTestKit/Sources/SupraBench/SupraBenchCommand.swift",
